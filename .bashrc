...
export REPO_CMD="reporter repo --trace --trace-to-stderr --time --no-pager"	; alias repo="${REPO_CMD}"
...
alias doperms="_sync publish perms"
alias dwm-do="xdotool key Super+0 ; xdotool key Super+Shift+grave"
...
alias pics="feh -. -g 800x600 -S name"
...
alias quotes="cd data.personal ; ${EDITOR} _quotes.txt ; strfile _quotes.txt _quotes.txt.dat ; fortune _quotes.txt ; ${LL} -t"
...
function mount-zfs {
	...
	declare Z_DAT="zfs get -H -t filesystem -o value"
	declare Z_DAT_ALL="zfs get -t filesystem -o all all"
	declare Z_ZDB="zdb -l"
...
#>>>			echo -en "3"					>/proc/sys/vm/drop_caches
	...
	ZSTAT="${ZSTAT,,}"
	declare ZPINT_Z= ; FILE="0"
	until {
		[[ -n ${ZPINT_Z} ]] ||
		(( ${FILE} == ${#ZDEVS[@]} ));
	}; do
		ZPINT_Z="$(${Z_ZDB}		${ZDEVS[${FILE}]}		2>/dev/null | ${SED} -n "s|^[ ]{4}name[:][ ][\'](.+)[\']$|\1|gp"			)"
		FILE="$((${FILE}+1))"
	done
...
# 2020-04-07 22:52:14 +0200 cdd8edbbb68d6cecdced5036d8e3a3c8583ac512 Add a note about prebuilt server in BUILD.md
function scrcpy {
	declare VER="master"
	declare BLD="false"
	declare HLP="false"
...
function task-export-drive-sync {
	if [[ ${1} == todo ]]; then
		...
		${SED} -i \
			...
			-e "s|^([#]+[[:space:]])|\n\1|g" \
			-e "s|^([[:space:]]*)[-][[:space:]]|\1* |g" \
			-e "s|^([[:space:]]*)[0-9a-z]+[.][[:space:]]{1,2}|\11. |g" \
...
#>>>		if [[ "${@}" != "-" ]] && [[ "${FILE}" == *(*)[ ]*(*) ]]; then
		if [[ "${@}" != "-" ]] && [[ -n "$(echo "${FILE}" | ${GREP} "[ ]")" ]]; then
			task mark kind:track		"${FILE}"
			task view kind.not:track	"${FILE}"
		else
